...
def r_squared(y, estimated):
    ...
    """
    assert len(y) == len(estimated), "y and estimated do not have the same length as each other"
    numer = denom = 0
...
def evaluate_models_on_training(x, y, models):
    ...
    for i in range(len(models)):
        y_est = np.polyval(models[i], x)

        pylab.figure()
        pylab.plot(x, y, 'bo', label="Observed data")
        pylab.plot(x, y_est, 'r-', label="Predicted model")
        pylab.title("Temperature over Year (Model " + str(i+1) + ")\n" + 
                     r'$R^2$ = ' + str(r_squared(y, y_est)))
        pylab.xlabel("Year")
        pylab.ylabel("Temperature " + r'($^\circ C$)')
...
# Average annual temperature of Boston city
y = []
for year in INTERVAL_1:
